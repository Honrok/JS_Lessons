...
document.querySelector('.b-4').onclick = () => { div4.addEventListener('touchstart', t4); }
// ваше событие здесь!!!
...
a7.addEventListener('touchstart', t7);
// ваше событие здесь!!!
...
function t11(event) {
    console.log(event);
    let a = event.targetTouches[0].radiusX;
    let b = event.targetTouches[0].radiusY;
    out11.innerHTML = `radiusX: ${a} <br/> radiusY: ${b}`;
}
...
let div12 = document.querySelector('.div-12-max img');

function nextFunction() {
    for (let i = 0; i < images.length; i++) {
        if ((images[i].classList.contains('active-img') == true) && i != images.length - 1) {
            images[i].classList.remove('active-img');
            images[i + 1].classList.add('active-img');
            div12.setAttribute('src', images[i + 1].getAttribute('src'));
            break;
        } else if ((images[i].classList.contains('active-img') == true) && i == images.length - 1) {
            images[i].classList.remove('active-img');
            images[0].classList.add('active-img');
            div12.setAttribute('src', images[0].getAttribute('src'));
            break;
        }
    }
}
...
function prevFunction() {
    for (let i = 0; i < images.length; i++) {
        if ((images[i].classList.contains('active-img') == true) && i != 0) {
            images[i].classList.remove('active-img');
            images[i - 1].classList.add('active-img');
            div12.setAttribute('src', images[i - 1].getAttribute('src'));
            break;
        } else if ((images[i].classList.contains('active-img')) && i == 0) {
            images[i].classList.remove('active-img');
            images[images.length - 1].classList.add('active-img');
            div12.setAttribute('src', images[images.length - 1].getAttribute('src'));
            break;
        }
    }
}
