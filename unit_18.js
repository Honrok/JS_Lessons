...
function t3() {
    a3.forEach(elem => typeof (elem) == 'number' ? a3_res.push(elem) : 0);
    console.log(a3_res);
}
...
function t4() {
    sp.forEach(elem => a4_res.push(elem.getAttribute('data')));
    console.log(a4_res);
}
...
function t6() {
    a6_res = str6.split('');
    a6_res.forEach(elem => out6.textContent += elem + ', ');
...
function t7() {
    a7_res = str7.split(' ');
    a7_res.forEach(elem => out7.textContent += elem + ' ');
}
...
function t9() {
    let arr = [];
    a9.forEach(elem => elem.map(elem => arr.push(elem)));
    a9_res = arr.join('-');
    console.log(a9_res);
}
...
function t10() {
    ...
    // a10_res = a10_res.slice(0, -1); - Это варинат, если нужно убрать амперсанд в конце строки.
    console.log(a10_res);
}
